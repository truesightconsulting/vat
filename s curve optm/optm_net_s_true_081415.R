...
# Load in setup files
#######################################################################################
setwd("d:\\Users\\xzhou\\Desktop\\opt_0_08_14_2015_06_45_05_88957\\")
start=Sys.time()
  ...
  if (nrow(curve)==1){
    ...
    if (sum(curve$sp_current)>budget){
      ...
      optm <- cobyla(x0, fn, hin = hin, lower=a, upper=b,
                     nl.info = F, control = list(maxeval = 1000,ftol_abs=1,xtol_rel=1))
      par=optm$par/sum(optm$par)
      curve$sp_current=budget*par
      
      #######################################################################################
      # OPTM Rollup Random
      #######################################################################################
      curve.rollup=curve[,list(r_grs=sum(r_grs)),by=c(ex.shell.rollup$var)]
      result.random.shell=merge(shell,curve.rollup,by=c(ex.shell.rollup$var))
      # compute random scenario
      for.ran=result.random.shell[order(media_num)]
      no.media=1:nrow(result.random.shell)
      
      r.dupe.box=rep(0,nrow(result.random.shell))
      r.dupe.box.media=matrix(0,nr=nrow(result.random.shell),
                              nc=nrow(result.random.shell),
                              dimnames=list(for.ran$media_num,
                                            1:nrow(result.random.shell)))
      r.dupe.box.media[,1]=for.ran$r_grs
      
      for (j in 2:nrow(result.random.shell)){
        #j=2
        combo=combn(no.media,j)
        r.combo=matrix(for.ran$r_grs[combo],nr=j)
        r.combo.prod=apply(r.combo,2,prod)
      ...
      r_net_ran_total=sum(for.ran$r_grs)+sum((-1)^(1:nrow(result.random.shell)+1)*r.dupe.box)
      for.ran$r_net=apply(sweep(r.dupe.box.media,MARGIN=2,(-1)^(1:nrow(result.random.shell)+1),"*"),
                          1,sum)
      for.ran$r_dupe=for.ran$r_grs-for.ran$r_net
      r_dupe_ran_total=r_net_ran_total-sum(for.ran$r_net) #zx
      for.ran=merge(for.ran[,!"sp_current",with=F],curve[,c("sp_current","media_num"),with=F],by="media_num")
      
      #######################################################################################
      # OPTM Rollup Max
      #######################################################################################
      curve.rollup=curve[,list(r_grs=sum(r_grs)),by=c(ex.shell.rollup$var)]
      result.max.shell=merge(shell,curve.rollup,by=c(ex.shell.rollup$var))
      ...
      for.max=merge(for.max[,!"sp_current",with=F],curve[,c("sp_current","media_num"),with=F],by="media_num")
      ...
      for.ran1=for.ran[,!c("media_num","shell_num","max_spend","min_spend","min_spend_t",
                           "inf_point1","r_grs"),with=F]
      ...
      index=ex.dupe$Media %in% curve$Media
      dupe=ex.dupe[index]
