...
ex.curve=merge(ex.curve,dupe.media,by="media_num",all.x=T)
ex.curve$dupe[is.na(ex.curve$dupe)]=0
ex.curve$dupe.f=1.2*exp(-ex.curve$dupe)
ex.curve$g.old=ex.curve$g
ex.curve$g=ex.curve$g*ex.curve$dupe.f
...
if (nrow(curve)==0){
  ...
  if (nrow(curve)==1){
    ...
    if (sum(curve$sp_current)>budget){
      ...
      # replace gross reach by using the original curves
      curve=curve[,g.old1:=g.old/cpp]
      curve.temp=curve[,c("media_num","g.old1","k","v","sp_current"),with=F]
