setwd("c:\\Users\\XinZhou\\Desktop\\vat curves\\curves")
library(data.table)
file=dir(getwd())

for (loop in file){
  print(loop)
  data=fread(loop,skip=4,header=F)
  shell=fread(loop,nrows=4,header=F)
  g.start=1e-3
  v.start=0.5
  ...
  write.csv(shell,paste("output_",loop,sep=""),row.names=F)
}
